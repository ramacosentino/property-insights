  ...
  const thresholdContent = (
    <div>
      <div className="flex items-center justify-between mb-2">
        <span className="text-[11px] font-medium text-foreground">Mostrar propiedades con m² barato</span>
      ...
      <Slider
        ...
        max={80}
        step={5}
        value={[80 - dealThreshold]}
        onValueChange={(v) => setDealThreshold(80 - v[0])}
        className="w-full"
      />
      <div className="flex items-center justify-between mt-1">
        <span className="text-[9px] text-muted-foreground">-80% más barato</span>
        <span className="text-[9px] text-muted-foreground">0% (precio de mercado)</span>
      </div>
      <p className="text-[9px] text-muted-foreground mt-1.5">
        {dealProperties.length} propiedad{dealProperties.length !== 1 ? "es" : ""} al menos {dealThreshold}% más barata{dealProperties.length !== 1 ? "s" : ""} que su zona
      </p>
    </div>
  );
